...
class Motion:
  ...
  def __motionDistributionToLocationDistribution(self, distribution, location):
    ...
    for direction in distribution.keys():
      ...
      clippedDistribution[(row, col)] += prob
...
class GameState:
  ...
  def elapseTime(self, sensors, motion=None):
    ...
    for location in self.layout.locations:
      row, col = location
      self.sensorReadings[row][col] = sampleFromCounter(sensors.getReadingDistributionGivenGhostTuple(self.ghostTuple, location))
...
class Game:
  ...
  def getGhostTuples(self):
    ...
    """
    tuples = [[]]
    for i in range(self.getNumGhosts()):
      newTuples = []
      for t in tuples:
        for l in self.getLocations():
          newTuples.append(t + [l])
      tuples = newTuples
    return [tuple(i) for i in tuples]
  ...
  def getInitialDistribution(self):
    ...
    for ghostTuple in ghostTuples:
      ...
      p_GhostTuples[ghostTuple] = distanceToCenter
  ...
  def run(self):
    ...
    while True:
      ...
      if action[0] == 'wait':
        ...
        self.display.infoPane.updateScore(self.state, self.busts)
        self.display.endGame(bustResults, self.state.ghostTuple)
        return True
...
USAGE_STRING = """
  ...
  OPTIONS:    --layout, -l
                  ...
                  Legal values: test, tiny, small, medium, large
              --ghosts, -k
              ...
              --sensortype, -s
                  ...
                  legal values: deterministic, noisy, simple
