...
class StaticVPIAgent(StaticGhostbusterAgent):
  ...
  def getAction(self):
    ...
    allLocations = list(self.game.getLocations())
    numGhosts = self.game.getNumGhosts()
    allBustingOptions = self.game.getBustingOptions()
    currentObservations = self.observations
    
    expectedUtilities = Counter()
    
    if len(currentObservations) > 0:
        for sensedLoc in currentObservations.keys():
            allLocations.remove(sensedLoc)
    
    for loc in allLocations:
        possibleReadings = self.inferenceModule.getReadingDistributionGivenObservations(currentObservations,loc)
        totalUtility = 0
        for reading in possibleReadings:
            currentObservations[loc] = reading
            expectedGhostCounts = self.getExpectedGhostCounts(currentObservations)
            currentObservations.pop(loc)
            bestBustTotal = -1
            for bust in allBustingOptions:
                bustTotal = 0
                for bustLoc in bust:
                    bustTotal += expectedGhostCounts[bustLoc]
                if bustTotal > bestBustTotal:
                    bestBustTotal = bustTotal
            totalUtility += bestBustTotal*possibleReadings[reading]
        expectedUtilities[loc] = totalUtility*GHOST_SCORE
    
    expectedMax, expectedMaxes = maxes(expectedUtilities)
    #print 'expected=',expectedMax
    
    'get current utility'
    currentUtility = Counter()
    expectedGhostCounts = self.getExpectedGhostCounts(currentObservations)
    ...
    for bust in allBustingOptions:
        ...
        for bustLoc in bust:
            bustTotal += expectedGhostCounts[bustLoc]
        currentUtility[bust] = bustTotal*GHOST_SCORE
    currentMax, currentMaxes = maxes(currentUtility)
    #print 'current=',currentMax
    #print expectedGhostCounts
    #print''
    
    if currentMax > expectedMax + SENSOR_SCORE:
        return Actions.makeBustingAction(random.choice(currentMaxes))
    
    return Actions.makeSensingAction(random.choice(expectedMaxes))
