...
(define (subst a-wae sub-id val)
  (type-case F1WAE a-wae
    ...
    [id (name) (cond
                 [(num? (find-arg-val name val)) (find-arg-val name val)]
                 [(and (F1WAE? (find-arg-val name val)) (not (empty? (find-arg-val name val)))) (find-arg-val name val)]
                 [(and (and (symbol? name) (symbol? sub-id)) (symbol=? name sub-id)) (num val)]
                 [else a-wae])]
